...
plat = 32  if sys.platform == 'win32' else 64  # 32 or 64
...
extensions = []
print("setup.py mode", sys.argv[1])
...
if not sys.argv[1] == 'install':
    ...
    bname32 = 'build86'
    bname64 = 'build64'
    build64_dir = pathlib.Path(__file__) / '..' / 'CascLib' / bname64
    build32_dir = pathlib.Path(__file__) / '..' / 'CascLib' / bname32
